...
- Workbench Net API is permanently enabled. If `wb_connect` fails, immediately inspect the latest Workbench `error.log` before retrying or restarting: in most cases a Game/Workbench script compilation error prevents the EnfusionMCP handler addon from loading and leaves the bridge unavailable even while the Workbench process and its Net API port are running. Use `console.log` for additional engine context when `error.log` is insufficient. Only after compilation is clean investigate Workbench startup or handler-addon loading; do not ask to enable Net API.
