...
3. При необходимости измените список: один путь `.ent` на строку, пустые строки и строки с `#` игнорируются, повторы убираются. Используйте полноценные сценарии, а не terrain-миры.
4. Запустите команду повторно. Она последовательно вызывает SetOpenedResource, проверяет фактический GetWorldPath и запускает существующий аудит. Отказ загрузки/отмена, несовпадение пути или незавершённый аудит останавливают пакет; оставшиеся миры не считаются проверенными.
5. Отчёт `$profile:ME_AmbientAuditBatch_N.txt` содержит результаты и coverage каждой карты и количество обработанных сценариев. Номер выбирается свободный, старые отчёты не перезаписываются. `$profile:` — профиль текущего Workbench.

Лог: `[ME_DEBUG_AVSP_BATCH] status=WORLD_STARTED world=...` и WORLD_FINISHED обрамляют подробные `[ME_DEBUG_AVSP_AUDIT]` сообщения. Файловый отчёт содержит сводки и раздел DETAILS каждой карты: POINT с координатами/ID, затем все ERROR, WARNING и UNAVAILABLE этой точки. Лог сохраняет те же сообщения. FINISHED пакета означает завершённый обход списка, а не отсутствие ошибок на картах.

Пакет работает синхронно, поэтому загрузка крупных миров может задерживать интерфейс. Если API возвращает управление до подтверждения нужного мира, пакет остановится с loaded_world_not_confirmed; результаты старой карты не присваиваются новой. Пакет не расширяет гарантии полноты существующего аудита: UNVERIFIED сохраняется отдельно.

Проверка Workbench ещё не выполнена: проверить Reload WB Scripts, создание списка, последовательность Cain → Eden, отсутствие смешения миров (имя текущего мира и число точек), отчёт по двум сценариям, сохранность старых отчётов. Отдельно проверить неверный путь и отмену открытия мира: ожидается ABORTED, без успешной строки для неподтверждённого мира. После этого сравнить сводки с одиночным аудитом тех же миров. Автоматизация запуска Workbench в текущей сессии недоступна.

### Подтверждённый пакет Cain → Eden

2026-09-24 10:26:06.891–10:27:06.905: ME_AmbientAuditBatch_1.txt, FINISHED completed=2 requested=2. Cain: scanned=62, passed=17, errorPoints=17, warningPoints=43, clearanceErrorPoints=12, intersections=169; результат совпал с одиночным аудитом. Eden: scanned=171, passed=85, errorPoints=6, warningPoints=86, clearanceErrorPoints=6, intersections=270. На обеих картах unavailablePoints=0, SOURCE_CHECK=MATCH (62/62 и 171/171), unresolvedPointSources=0. Coverage=UNVERIFIED сохраняется из-за прочих source (3371 и 2606). Последовательная загрузка и запись отчёта подтверждены логом; проверка отмены/неверного пути и независимый одиночный прогон Eden пока не выполнены.

Подробный отчёт добавлен после первого успешного пакета. Тексты ошибок фильтра содержат include/exclude и фракцию; геометрические предупреждения — координаты точки и объекта, его ID и локальные габариты. Буфер очищается перед каждой точкой и каждым миром. Повторная компиляция и проверка записи DETAILS в Workbench ещё требуются. Первый пакет Cain/Eden восстановлен отдельно из script.log без повторной загрузки карт.

Список пакетного запуска расширен по скриншотам пользователя до семи сценариев: Cain, Eden, Arland, HQC_Eden, Montignac, WesternEveron, HQC_Arland. Для добавленных ресурсов используется каталог worlds/MP, как у Cain/Eden; фактическая загрузка новых путей ещё должна быть подтверждена Workbench. Обновлены существующий пользовательский манифест и шаблон для новых профилей.

### Prefab точки в аудите

Диагностика каждой точки и заголовки POINT в подробном пакетном отчёте содержат `pointPrefab` (например, `AmbientVehicleSpawnpoint_CIV`) и `pointPrefabPath` (полный ресурс). Значения берутся из prefab самой сущности, а не определяются по фракции. Если prefab недоступен, записывается `<unavailable>`.

Для получения этих полей нужно перезагрузить WB Scripts и повторить аудит. В старых логах и CSV prefab отсутствует; достоверно восстановить его только по фракции нельзя. Проверка компиляции и выполнения в Workbench ожидается.

Проверка Batch 3 (24.09.2026): завершены 7/7 миров, итоговые счётчики совпали с Batch 2. Все 956 записей диагностики получили pointPrefab=<unavailable>. Добавлено получение ближайшего .et через GetResourceName()/GetAncestor() источника редактора; GetPrefabData() оставлен резервным вариантом. Путь передаётся на время проверки точки и затем очищается. Изменение требует повторной проверки в Workbench; CSV с prefab пока не обновлены.

Подтверждено Batch 4 (24.09.2026, Arma Reforger 1.8.0.13): 7/7 миров завершены, во всех 956 записях диагностики заполнены pointPrefab и pointPrefabPath, unavailable отсутствует. Получены CIV, FIA и USSR; счётчики точек, ошибок и предупреждений совпали с Batch 2/3. Подготовлены CSV по мирам и отдельные CSV ошибок с колонками Prefab/PrefabPath; проверены количество строк после импорта, число предупреждений и уникальных точек с ошибками. Montignac и WesternEveron сохраняют NO_POINTS, покрытие остаётся UNVERIFIED.


## Сравнение между версиями игры

Эталон 1.8.0.13, сводка и инструкция: [audits/1.8.0.13/Summary.md](audits/1.8.0.13/Summary.md). Скрипт tools/Compare-AmbientAudit.ps1 сравнивает завершённые пакетные отчёты по счётчикам, покрытию и замечаниям с координатами/prefab. Проверены совпадающий отчёт, перенос точки при неизменных счётчиках и отклонение прерванного отчёта.

Эталон Batch 4 разделён на docs/audits/1.8.0.13/worlds/*.txt. Сравнение исходного пакета с объединёнными файлами дало ноль изменений. Compare-AmbientAudit.ps1 поддерживает общий файл и каталог отчётов.

Текущий формат: pointPrefabPath удалён из вывода и сохранённых эталонов по просьбе пользователя; остаётся pointPrefab. Путь используется внутри для определения имени. Compare-AmbientAudit.ps1 сравнивает короткое имя и принимает старые отчёты с дополнительным полем пути.

## Пересечения между точками в массовом аудите

Добавлен ERROR reason=overlapping_spawn_area с coordinates и overlappingCoordinates. Проверяется расстояние между центрами в 3D <= 2 * SPAWNING_RADIUS, включая касание, как в редакторской подсказке. Все доступные точки собираются до проверки, независимо от регистрации preview. Для каждой точки выводятся все соседи; пара появляется в двух направлениях, errorPoints считает каждую проблемную точку один раз. Это конфликт размещения, а не гарантия отказа runtime-спавна.

Compare-AmbientAudit.ps1 учитывает overlappingCoordinates в ключе замечания. Проверка подтверждена Batch 6, включая указанную пару HQC Eden. Актуальный эталон заменил ранний снимок без проверки пересечений.
Batch 6 подтверждён 24.09.2026: 7/7 миров; overlapping_spawn_area — Cain 1 пара, Eden 1, HQC Eden 4. Ошибочных точек соответственно 19, 8, 18; остальные счётчики ошибок и предупреждений не изменились. Эталон и сравнение: audits/1.8.0.13/Summary.md. Полный и фильтрованный эталоны сравниваются с исходным пакетом без расхождений.
