...
4. Сохраняйте production-правила документации: описание скрипта, класса, enum и каждого поддерживаемого метода должно быть на английском, а сразу после него — русский вариант.
5. Для modded overrides сохраняйте `super`-вызовы, если изменение явно не заменяет базовое поведение.
6. После переноса откройте production `addon.gproj` в Workbench, перезагрузите скрипты и проверьте свежий `error.log`.
